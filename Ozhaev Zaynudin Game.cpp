...
void CollisionBall (Ball* plaer, Ball* ball)
    {
	 ...
	 if (hypotenuse < (*ball) .r + (*plaer) .r) //проверка столкновения
        ...
        double dt_Collision = ((*ball) .r + (*plaer) .r - hypotenuse)/(Vn1 - Vn2); //удаление залипания

        if (dt_Collision >  0.7) dt_Collision =  0.7; //ограничение на dt, чтобы мяч не отскакивал далеко
